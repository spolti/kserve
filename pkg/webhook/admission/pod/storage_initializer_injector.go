...
func (mi *StorageInitializerInjector) InjectStorageInitializer(pod *v1.Pod) error {
	...
	fmt.Printf("\nISVC ANN: %v\n", isvcReadonlyString)
	fmt.Printf("FORA AaADASDASDASDASDASDASDASDASD %s", pod.ObjectMeta.Annotations[constants.StorageReadonlyAnnotationKey])
	if ok {
		fmt.Printf("AaADASDASDASDASDASDASDASDASD %s", pod.ObjectMeta.Annotations[constants.StorageReadonlyAnnotationKey])
