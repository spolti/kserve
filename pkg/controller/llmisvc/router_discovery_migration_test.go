...
func TestIsInferencePoolV1Alpha2Supported(t *testing.T) {
	tests := []struct {
		...
		{
			name: "route with v1 InferencePool backendRef returns Unknown (not v1alpha2)",
			route: HTTPRoute("test-route",
				InNamespace[*gatewayapi.HTTPRoute]("test-ns"),
				WithParentRef(GatewayRef("test-gateway", RefInNamespace("test-ns"))),
				WithHTTPRule(
					WithBackendRefs(BackendRefInferencePoolV1("my-pool")),
				),
				WithHTTPRouteReadyStatus("openshift.io/gateway-controller/v1"),
			),
			expected: metav1.ConditionUnknown,
		...
		{
			name: "route with v1alpha2 InferencePool and ResolvedRefs=True returns True",
			route: HTTPRoute("test-route",
				InNamespace[*gatewayapi.HTTPRoute]("test-ns"),
				WithParentRef(GatewayRef("test-gateway", RefInNamespace("test-ns"))),
				WithHTTPRule(
		...
		{
			name: "route with v1alpha2 InferencePool and ResolvedRefs=False InvalidKind returns False",
			route: HTTPRoute("test-route",
				InNamespace[*gatewayapi.HTTPRoute]("test-ns"),
				WithParentRef(GatewayRef("test-gateway", RefInNamespace("test-ns"))),
				WithHTTPRule(
		...
		{
			name: "route with v1alpha2 InferencePool and ResolvedRefs=False BackendNotFound returns True (kind is supported)",
			route: HTTPRoute("test-route",
				InNamespace[*gatewayapi.HTTPRoute]("test-ns"),
				WithParentRef(GatewayRef("test-gateway", RefInNamespace("test-ns"))),
				WithHTTPRule(
		...
		{
			name: "route with v1alpha2 InferencePool and no status returns Unknown",
			route: HTTPRoute("test-route",
				InNamespace[*gatewayapi.HTTPRoute]("test-ns"),
				WithParentRef(GatewayRef("test-gateway", RefInNamespace("test-ns"))),
				WithHTTPRule(
