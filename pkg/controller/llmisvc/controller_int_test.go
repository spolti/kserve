...
var _ = Describe("LLMInferenceService Controller", func() {
	...
	Context("Custom Gateway Reference", func() {
		It("should not require default gateway when custom gateway is specified", func(ctx SpecContext) {
			...
			defaultGateway := &gatewayapi.Gateway{}
			...
			Eventually(func(g Gomega, ctx context.Context) error {
				...
				g.Expect(&routes[0]).To(HaveGatewayRefs(gatewayapi.ParentReference{
					Name:      gatewayapi.ObjectName(customGatewayName),
			...
			ensureHTTPRouteReady(ctx, envTest.Client, llmSvc, createdRoute)
			...
			Eventually(func(g Gomega, ctx context.Context) error {
				...
				routerCondition := current.Status.GetCondition(v1alpha1.RouterReady)
