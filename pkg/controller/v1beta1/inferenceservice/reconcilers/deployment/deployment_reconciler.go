...
func addOauthContainerToDeployment(ctx context.Context,
	...
	if val, ok := componentMeta.Annotations[constants.ODHKserveRawAuth]; ok && strings.EqualFold(val, "true") {
		...
		oauthProxyContainer, err := generateOauthProxyContainer(ctx, client, clientset, isvcName, componentMeta.Namespace, upstreamPort, upstreamTimeout, podSpec.ServiceAccountName)
		if err != nil {
...
func generateOauthProxyContainer(ctx context.Context, client kclient.Client, clientset kubernetes.Interface, isvc string,
	namespace string, upstreamPort string, upstreamTimeout string, podSpecSA string,
	...
	// Dashboard deploys a service account with the same name as the inferenceservice, we need to check it as well.
	dashboardSaName := isvc + "-sa"
	serviceAccount, msg := utils.GetServiceAccountName(ctx, clientset, namespace, dashboardSaName, podSpecSA)
	log.V(2).Info("ServiceAccount: ", "name", serviceAccount, "msg", msg)
...
func createSarCm(ctx context.Context, client kclient.Client, clientset kubernetes.Interface, namespace string, inferenceServiceName string) error {
  ...
  resourceAttributes:
	...
	if err != nil {
		...
		// Update existing configmap if content differs
		if existingConfigMap.Data["config-file.yaml"] != configContent {
			_, err = clientset.CoreV1().ConfigMaps(namespace).Update(ctx, sarConfigMap, metav1.UpdateOptions{})
			if err != nil {
				return fmt.Errorf("failed to update SAR configmap: %w", err)
			}
			log.V(2).Info("Updated SAR ConfigMap", "name", configMapName, "namespace", namespace)
		}
	}
	return nil
}
