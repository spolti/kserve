...
// InjectPredictorTracing injects OTEL tracing env vars and, for vLLM-backed
// predictors, CLI args into a predictor container. Returns true if the
// container was mutated.
//
// variant identifies the canary variant name (empty for the primary predictor).
// isVLLM should be true when the predictor uses vLLM as its serving runtime.
func InjectPredictorTracing(t *v1beta1.TracingSpec, namespace, isvcName, variant string, isVLLM bool, container *corev1.Container) bool {
	if t == nil {
		return false
	}
