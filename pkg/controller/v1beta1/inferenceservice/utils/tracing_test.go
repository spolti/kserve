...
func TestInjectPredictorTracing(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "empty TracingSpec injects env vars with zero-value fields",
			spec:      &v1beta1.TracingSpec{},
			namespace: "test-ns",
			isvcName:  "my-isvc",
			container: &corev1.Container{Name: "main"},
			...
			check: func(g *GomegaWithT, c *corev1.Container) {
				...
				g.Expect(envMap).To(HaveKeyWithValue(tracing.EnvOtelServiceName, "my-isvc-predictor"))
				g.Expect(envMap).To(HaveKeyWithValue(tracing.EnvOtelExporterEndpoint, ""))
				g.Expect(envMap).To(HaveKeyWithValue(tracing.EnvOtelTracesExporter, ""))
				g.Expect(envMap).To(HaveKeyWithValue(tracing.EnvOtelTracesSampler, ""))
				g.Expect(envMap).To(HaveKeyWithValue(tracing.EnvOtelTracesSamplerArg, ""))
				g.Expect(envMap).To(HaveKey(tracing.EnvOtelResourceAttributes))
				g.Expect(envMap[tracing.EnvOtelResourceAttributes]).To(ContainSubstring("k8s.namespace.name=test-ns"))
				g.Expect(envMap[tracing.EnvOtelResourceAttributes]).To(ContainSubstring("isvc.name=my-isvc"))
				g.Expect(envMap[tracing.EnvOtelResourceAttributes]).To(ContainSubstring("isvc.component=predictor"))
				g.Expect(c.Args).To(BeEmpty())
			},
		},
		{
		...
		{
			name:      "vLLM predictor without endpoint skips CLI args",
			spec:      &v1beta1.TracingSpec{},
			namespace: "ns",
			isvcName:  "vllm-isvc",
			isVLLM:    true,
		...
		},
		{
			...
			check: func(g *GomegaWithT, c *corev1.Container) {
				...
				InjectPredictorTracing(fullTracingSpec(), "ns", "my-isvc", "", true, c)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewGomegaWithT(t)
			mutated := InjectPredictorTracing(tt.spec, tt.namespace, tt.isvcName, tt.variant, tt.isVLLM, tt.container)
