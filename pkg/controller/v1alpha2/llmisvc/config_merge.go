...
func ReplaceVariables(llmSvc *v1alpha2.LLMInferenceService, llmSvcCfg *v1alpha2.LLMInferenceServiceConfig, reconcilerConfig *Config) (*v1alpha2.LLMInferenceServiceConfig, error) {
	...
	compactOptionalTemplateValues(
		reflect.ValueOf(llmSvcCfg),
		reflect.ValueOf(out),
		llmSvcCfg.Spec.KVCacheOffloading == nil,
	)
	return out, nil
}

// compactOptionalTemplateValues removes empty arguments and shell continuation
// lines generated by disabled optional templates. It compares the source and
// rendered values so explicit empty arguments in legacy templates are preserved.
func compactOptionalTemplateValues(source, rendered reflect.Value, compactEmptyKVTransfer bool) {
	if !source.IsValid() || !rendered.IsValid() {
		return
	}
	if source.Kind() == reflect.Pointer && rendered.Kind() == reflect.Pointer {
		if !source.IsNil() && !rendered.IsNil() {
			compactOptionalTemplateValues(source.Elem(), rendered.Elem(), compactEmptyKVTransfer)
		}
		return
	}
	if source.Kind() == reflect.Struct && rendered.Kind() == reflect.Struct {
		for i := range min(source.NumField(), rendered.NumField()) {
			sourceField := source.Field(i)
			renderedField := rendered.Field(i)
			if rendered.Type().Field(i).Name == "Args" && renderedField.CanSet() && renderedField.Type() == reflect.TypeOf([]string{}) {
				sourceArgs := sourceField.Interface().([]string)
				renderedArgs := renderedField.Interface().([]string)
				compacted := make([]string, 0, len(renderedArgs))
				changed := false
				for j, arg := range renderedArgs {
					if arg == "" && j < len(sourceArgs) && isCompactableTemplate(sourceArgs[j], compactEmptyKVTransfer) {
						changed = true
						continue
					}
				...
				if changed {
					renderedField.Set(reflect.ValueOf(compacted))
				}
				continue
			}
			if rendered.Type().Field(i).Name == "Command" && renderedField.CanSet() && renderedField.Type() == reflect.TypeOf([]string{}) {
				sourceCommands := sourceField.Interface().([]string)
				renderedCommands := renderedField.Interface().([]string)
				for j := range min(len(sourceCommands), len(renderedCommands)) {
					if !containsOptionalTLSTemplate(sourceCommands[j]) {
						continue
					}
					lines := strings.Split(renderedCommands[j], "\n")
					lines = slices.DeleteFunc(lines, func(line string) bool { return strings.TrimSpace(line) == `\` })
					renderedCommands[j] = strings.Join(lines, "\n")
				}
				renderedField.Set(reflect.ValueOf(renderedCommands))
				continue
			}
			compactOptionalTemplateValues(sourceField, renderedField, compactEmptyKVTransfer)
		}
		return
	}
	if source.Kind() == reflect.Slice && rendered.Kind() == reflect.Slice {
		for i := range min(source.Len(), rendered.Len()) {
			compactOptionalTemplateValues(source.Index(i), rendered.Index(i), compactEmptyKVTransfer)
		}
	}
}

func isCompactableTemplate(value string, compactEmptyKVTransfer bool) bool {
	return containsOptionalTLSTemplate(value) ||
		(compactEmptyKVTransfer && strings.Contains(value, "{{ kvTransferConfig "))
}

func containsOptionalTLSTemplate(value string) bool {
	return strings.Contains(value, ".GlobalConfig.EnableTLS") ||
		strings.Contains(value, ".GlobalConfig.TLSMinVersion") ||
		strings.Contains(value, ".GlobalConfig.TLSCipherSuites")
}
