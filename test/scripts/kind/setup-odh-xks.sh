...
#   KSERVE_CONTROLLER_IMAGE=kserve-controller:dev
#
...
KSERVE_CONTROLLER_IMAGE="${KSERVE_CONTROLLER_IMAGE:-kserve-controller:dev}"
...
build_and_load_controller() {
  ...
  IMG="${KSERVE_CONTROLLER_IMAGE}" make docker-build
...
deploy_odh_xks() {
  ...
  kubectl patch deployment kserve-controller-manager \
    -n "${KSERVE_NAMESPACE}" \
    --type=json \
