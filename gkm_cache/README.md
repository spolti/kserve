...
### Target Namespace SCC

GKM extraction jobs require the `anyuid` SCC on the `default` service account in the
**target namespace** (where the GKMCache CR and production ISVC will run). The extraction
init container runs as root (UID 0) and needs to `chown` the cache PVC mount.

```bash
# Grant anyuid to default SA in the target namespace
oc adm policy add-scc-to-user anyuid -z default -n <target-namespace>
```

**Do NOT grant `anyuid` in the build namespace** -- it breaks OCI ModelCar storage.
See [Troubleshooting: anyuid SCC Conflicts](#anyuid-scc-conflicts-with-oci-modelcar) for details.

### SELinux Context for GKM hostPath Volumes

On nodes with GPUs, the GKM agent creates **hostPath PVs** at `/tmp/gkm` instead of using
the CSI driver. The default SELinux context on this directory (`container_var_run_t`) prevents
extraction pods from accessing the mount, even with `anyuid` SCC.

You must fix the SELinux context on each GPU node before running the pipeline:

```bash
# Fix SELinux context on each GPU node
oc debug node/<gpu-node> -- chroot /host chcon -R -t container_file_t /tmp/gkm
```

This is a known GKM agent issue -- the agent should set the correct SELinux context when
creating the directory. See [Troubleshooting: GKM hostPath Permission Denied](#gkm-hostpath-permission-denied)
for symptoms and diagnosis.

### GKM Operator

The [GKM (GPU Kernel Manager)](https://github.com/redhat-et/GKM) operator must be installed
in the cluster. GKM manages the lifecycle of GPU kernel caches -- extracting OCI cache images
into per-node PVCs so that InferenceServices can mount them at serving time.

```bash
# 1. Install cert-manager (required by GKM)
kubectl apply -f https://github.com/cert-manager/cert-manager/releases/latest/download/cert-manager.yaml

# 2. Install GKM operator
git clone https://github.com/redhat-et/GKM.git && cd GKM && make deploy
```

The pre-flight check task verifies that the `gkmcaches.gkm.io` CRD is present before
the pipeline starts.

### Kyverno Image Verification (Optional)

If your cluster uses [Kyverno](https://kyverno.io/) to enforce image signature verification,
you need a `ClusterPolicy` that allows GKM to pull the signed cache images. The pipeline
signs cache images with [cosign](https://github.com/sigstore/cosign) (key-based or keyless),
and Kyverno can verify those signatures before GKM extracts them.
...
Example Kyverno policy for cosign key-based verification:

```yaml
apiVersion: kyverno.io/v1
kind: ClusterPolicy
metadata:
  name: verify-gkm-cache-images
spec:
  ...
  - name: verify-cosign-signature
    ...
    verifyImages:
    - imageReferences:
      - "quay.io/yourorg/gkm_cache_container:*"
      attestors:
      - entries:
        - keys:
            publicKeys: |-
...
For keyless signing (Sigstore Fulcio), adjust the `attestors` section to use
`keyless` with the appropriate OIDC issuer and subject identity.

The GKMCache provisioner task stores the signature format (`cosign-v2`, `cosign-v3`,
or `none`) as a label on the GKMCache CR (`gkm.io/signature-format`) for traceability.

### Cluster Requirements
- **Kubernetes**: v1.23+
- **Tekton Pipelines**: v0.50+
- **ODH/RHOAI**: Open Data Hub or Red Hat OpenShift AI operator installed
- **GKM Operator**: GPU Kernel Manager operator with cert-manager
...
- **Kyverno**: Optional, for image signature verification policies
...
| `mcv-image` | No | `quay.io/gkm/mcv:latest` | Pre-built [GKM MCV](https://github.com/redhat-et/GKM/blob/main/mcv/README.md) image for cache packaging |
| `cosign-secret` | No | `cosign-signing-key` | Cosign private key secret for image signing (keyless if absent) |
| `storage-class-name` | Yes | - | StorageClass for the GKMCache PVC (cluster-specific) |
| `target-namespace` | Yes | - | Namespace where GKMCache CR and production ISVC will run |
...
**`cosign-secret`**: Name of a Kubernetes secret containing a [cosign](https://github.com/sigstore/cosign)
private key for signing the cache image. The secret must contain a `cosign.key` data entry
and optionally `cosign.password`. If the secret does **not** exist in the namespace, the
pipeline automatically falls back to **keyless signing** via [Sigstore](https://www.sigstore.dev/)
(Fulcio + Rekor). The signature format (`cosign-v2`, `cosign-v3`, or `none`) is passed to
the GKMCache provisioner task so that GKM can verify the image before extraction.

**`storage-class-name`**: The StorageClass used for the GKMCache PVC in the target namespace.
Must match an available StorageClass in the cluster (e.g., `gp3-csi` on AWS, `ocs-storagecluster-cephfs`
on ODF, `managed-premium` on Azure).

**`target-namespace`**: The namespace where the GKMCache CR and production InferenceService
will run. The pipeline creates this namespace if it doesn't exist and instantiates the
ServingRuntime there.
...
5. **[Image Packager](task-cache-image-packager.yaml)**
   ...
   - Signs the image with [cosign](https://github.com/sigstore/cosign) (key-based via `cosign-secret`, or keyless via Sigstore if no key is found)
   - Outputs `signature-format` result for downstream verification (e.g., Kyverno policies, GKMCache CR labels)

6. **[GKMCache Provisioner](task-gkm-cache-provisioner.yaml)**
   - Creates the target namespace if it doesn't exist
   - Instantiates the ServingRuntime from the template
   - Creates a GKMCache CR to provision cache PVCs via GKM
   - Waits for GKM extraction to complete (condition type: `Extracted`)
...
A ready-to-use InferenceService manifest is provided in
[`isvc-with-gkm-cache.yaml`](isvc-with-gkm-cache.yaml). Update the `claimName` to match
the PVC created by your GKMCache CR, then apply it in the target namespace.
...
#### `anyuid` SCC Conflicts with OCI ModelCar

GKM extraction jobs run as UID 1000 and require the `anyuid` SCC on the `default` service
account. However, KServe predictor pods also use the `default` service account, and granting
`anyuid` to it breaks OCI ModelCar storage.

**How it breaks:** OpenShift's ModelCar implementation uses a `/proc/PID/root` symlink to
access model weights from the storage-initializer container. When `anyuid` SCC is granted,
it overrides the `restricted-v2` SCC that ModelCar relies on. The predictor pod starts with
different security settings, causing `HFValidationError` because vLLM cannot access the
model files through the expected path.

**Symptom:** The predictor pod fails with:
```
huggingface_hub.errors.HFValidationError: Repo id must be in the form 'repo_name' or 'namespace/repo_name'
```
This happens because the `oci://` storage URI can't be resolved when the ModelCar mechanism
is broken by the wrong SCC.

**How the pipeline handles it:** The pipeline builds the cache in the `test-gkm-builder`
namespace (where `anyuid` is NOT granted to the `default` SA), and the GKMCache provisioner
task creates the GKMCache CR in a separate `target-namespace`. The `anyuid` SCC is granted
only in the target namespace where GKM extraction jobs run -- not in the build namespace.

**If you hit this issue:**
```bash
# Check which SCC the predictor pod is running under
oc get pod <predictor-pod> -o jsonpath='{.metadata.annotations.openshift\.io/scc}'
# Should be: restricted-v2 (NOT anyuid)

# Remove anyuid from default SA in the build namespace if accidentally granted
oc adm policy remove-scc-from-user anyuid -z default -n <build-namespace>
```

**Key rule:** Never grant `anyuid` to the `default` service account in a namespace where
KServe InferenceServices with OCI ModelCar storage (`oci://` URIs) will run. The pipeline
separates build and serving namespaces to avoid this conflict.

#### GKM hostPath Permission Denied

On GPU nodes, the GKM agent creates hostPath PVs at `/tmp/gkm` (instead of using the CSI driver).
The extraction job's `fix-permissions` init container fails because SELinux blocks access:

```
chown: cannot read directory '/cache': Permission denied
```

**Root cause:** The GKM agent creates `/tmp/gkm` with SELinux context `container_var_run_t`,
which does not allow container processes to access the directory. The extraction init container
runs as root (UID 0) with `anyuid` SCC, but SELinux denies access regardless of UID.

**Diagnosis:**
```bash
# Check SELinux context on the GPU node
oc debug node/<gpu-node> -- chroot /host ls -laZ /tmp/gkm
# Bad:  container_var_run_t
# Good: container_file_t

# Check GKMCache status
oc get gkmcache <name> -n <namespace> -o jsonpath='{.status.conditions}'
# Will show type=Error with "An error occurred trying to extract the Kernel Cache"

# Check which PVs are hostPath vs CSI
oc get pv -l cache-name=<gkmcache-name> -o custom-columns='NAME:.metadata.name,NODE:.metadata.labels.node,HOSTPATH:.spec.hostPath.path,CSI:.spec.csi.driver'
```
...
# Then delete and recreate the GKMCache CR to trigger fresh extraction
oc delete gkmcache <name> -n <namespace>
# Clean up stale hostPath PVs (they have Retain policy)
oc delete pv -l cache-name=<gkmcache-name>
# Recreate
oc apply -f <gkmcache-manifest>
```

**Note:** This is a GKM agent bug. The agent should set `container_file_t` when creating
the `/tmp/gkm` directory. Non-GPU nodes are unaffected because they get CSI-provisioned
EBS volumes instead of hostPath PVs.
...
**Action:** Consider adding a default `--compilation-config '{"mode": 3}'` to the
